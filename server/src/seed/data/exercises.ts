...
import { exerciseId, variantId } from "./ids.js";

export { exerciseId, variantId };
...
type VariantRecord = ExerciseVariant & { exerciseId: mongoose.Types.ObjectId };
...
export const mockExercises = PREDEFINED.map((row) => {
  ...
  return {
    ...
    variants: allVariants
      .filter((variant) => variant.exerciseId.equals(id))
      .map((variant) => ({
        _id: variant._id,
        name: variant.name,
...
export const getWorkoutExerciseSnapshot = (
  ...
  const catalogExerciseId = exerciseId(slug);
  const variant = allVariants.find(
    (item) =>
      item.exerciseId.equals(catalogExerciseId) && item.equipment === equipment,
  );

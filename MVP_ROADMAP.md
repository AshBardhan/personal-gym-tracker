...
**MVP scope (build order):** (1) workout + exercise pages with sample/MSW data, (2) server models/relationships and API for client integration, (3) auth with `user` / `admin` roles and ownership, then mobile polish, progress charts, admin management, and deploy (Atlas + static client + Node API).
...
- [ ] Workout pages with sample/MSW data: list, detail, create, edit aligned to catalog + variant + sets
- [ ] Exercise-related pages/UI: browse/search catalog; picker from MSW `GET /exercises` ([EXERCISES.md](EXERCISES.md))
- [ ] Domain types + MSW handlers matching server exercise/workout contracts
...
- [ ] Exercise catalog admin UI (name, category, primary/secondary muscles, default variant)
...
- [ ] Error boundaries and centralized Axios error messaging
...
- [ ] Exercise model + relationships + seed from [EXERCISES.md](EXERCISES.md); indexes
- [ ] Workout schema: ordered exercises with `exerciseId` + snapshots + variant + raw sets
- [ ] Workout/exercise API routes matching client MSW contracts (curl-verified)
- [ ] Seed script: demo user, exercise catalog, sample workouts as needed
- [ ] Env validation and typed config (`MONGODB_URI`, `PORT`, `NODE_ENV`)
- [ ] Central error middleware + consistent error JSON shape
- [ ] Request validation (users, workouts, exercises, ObjectIds)
...
- [ ] Query indexes verified in `mongosh`
...
- [ ] Freeze shared contracts (Exercise DTO, Workout write payload, error shape; auth token strategy later)
- [ ] Exercise catalog cutover: seed ↔ MSW ↔ form picker ↔ workout submit with `exerciseId`
- [ ] Workout create/update payload alignment (types, MSW, validation, form submit)
...
- [ ] Gate existing `/`, `/workouts*` behind auth (behavior change, same paths)
...
- [ ] `/exercises/:id/progress` — deep exercise history
...
- [Client README](client/README.md) — page routes implemented so far
- [Server README](server/README.md) — API routes implemented so far
